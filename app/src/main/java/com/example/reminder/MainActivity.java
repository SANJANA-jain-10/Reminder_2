...
public class MainActivity extends AppCompatActivity implements DatePickerDialog.OnDateSetListener, TimePickerDialog.OnTimeSetListener {
    ...
    List<Reminder_detail> todayList = new ArrayList<>();
        ...
        switch (requestCode) {
            case REQ_CODE_SPEECH_INPUT: {
                if (resultCode == RESULT_OK && null != data) {
                    ...
                    else if(stage_variable == 6){

                        String choice2 = result.get(0);
                        if (choice2.equals("yes")) {
                            for (Reminder_detail x : todayList
                            ) {
                                String repeat = "You have set a reminder at " + x.reminder_time + " hours for " + x.reminder_desc;
    ...
    private void remindtodaylist() {

        Intent intent = new Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH);
        intent.putExtra(RecognizerIntent.EXTRA_LANGUAGE_MODEL, RecognizerIntent.LANGUAGE_MODEL_FREE_FORM);
        intent.putExtra(RecognizerIntent.EXTRA_LANGUAGE, Locale.getDefault());
        intent.putExtra(RecognizerIntent.EXTRA_PROMPT, "Would you like to know today's reminders?");


        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            texttoSpeech.speak("Okay. Would you like to know today's reminders?", TextToSpeech.QUEUE_FLUSH, null, null);
        }
    ...
    public void addValueListener(){
        ...
        ValueEventListener vl = new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                LocalDateTime tomr = today1.plusDays(1);
                ...
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    ...
                    /*if(rd.reminder_f_date.equals(today)){
                        todayList.add(rd);
                    }*/
                ...
                for(int j=0;j<reminders.size();j++){
                    ...
                    if(sorted.reminder_f_date.compareTo(today1) >=0 && sorted.reminder_f_date.compareTo(tomr) < 0){
                        todayList.add(sorted);
                        System.out.println("***********Today************"+sorted.getReminder_desc());
                    }
